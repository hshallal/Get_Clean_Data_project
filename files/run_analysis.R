...
test <-  cbind(subject_test, y_test, X_test) ## test is a dataframe of the test set

subject_train<- read.table ("subject_train.txt")
y_train <- read.table ("y_train.txt")
X_train <- read.table ("X_train.txt")
train <-  cbind(subject_train, y_train, X_train) ## train is a dataframe of the train set
...
features_names <- (features[,2]) ## features_names is a factor, we need a character vector
features_Names <- as.character(features_names) ## features_Names is a character vector
...
extracted$subject <- as.factor(extracted$subject) ## factor with 30 levels
extracted$activitylabel <- as.factor(extracted$activitylabel) ## factor with 6 levels
